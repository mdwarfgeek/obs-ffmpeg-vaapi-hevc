...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("ffmpeg-vaapi-hevc", "en-US")
MODULE_EXPORT const char *obs_module_description(void)
{
        return "FFmpeg HEVC VAAPI plugin";
}
			 ...
			 struct encoder_packet *packet, bool *received_packet)
{
	...
	if (got_packet && enc->packet->size) {
		if (enc->first_packet) {
			...
			obs_extract_hevc_headers(enc->packet->data,
...
{
	const char *device = vaapi_get_hevc_default_device();
	...
	if (vaapi_device_rc_supported(VAProfileHEVCMain, va_dpy,
				      VA_RC_CBR, device))
		obs_data_set_default_string(settings, "rate_control", "CBR");
	else if (vaapi_device_rc_supported(VAProfileHEVCMain,
				  ...
				  obs_data_t *settings)
{
	...
	if (!va_dpy || !vaapi_display_hevc_supported(va_dpy, device))
		goto fail;
	...
	if (os_file_exists("/dev/dri/by-path")) {
		...
		while ((file = os_readdir(by_path_dir)) != NULL) {
			...
			if (strcmp(type, "render") == 0) {
				...
				if (!vaapi_device_hevc_supported(path))
					continue;
...
{
	fprintf(stderr, "obs-ffmpeg-vaapi loaded\n");
